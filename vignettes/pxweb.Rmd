...
pxweb_aggregation("agg_Alder5ar_1") # use an aggregation codelist
pxweb_valueset("vs_Alder1ar")       # use a value set codelist
```
...
The exact aggregation and value set identifiers are found in the v2 metadata for a table. In metadata returned by the API, they are listed under each variable's `extension$codelists` field.
