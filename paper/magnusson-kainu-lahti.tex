...
\author{by Måns Magnusson, Pyry Kantanen, Leo Lahti}
...
Opening up official statistics is only the first step towards realizing their full potential and value. Open data is necessary but no longer sufficient as focus has shifted away from simply opening data that was unavailable before towards qualitative aspects of open data: open data should now be Findable, Accessible, Interoperable, and Re-usable, or FAIR in short. In the European Union legislation these have even been codified in supranational legislation.
...
software languages. In R, various packages have been released
for generic open data retrieval, for instance
quandl \cite{quandl} and pdfetch \cite{pdfetch} for retrieving economic and time series data from multiple sources, as well as packages for downloading data from singular but vast data repositories such as
Eurostat \cite{Lahti17eurostat}, World Bank
(\CRANpkg{WDI}; \citealt{WDI}), and Open Street Map
(\CRANpkg{osmar}; \citealt{osmar}).

% Citing data is a problem in itself, but important. It should be
% simplifed as much as possible.

Data citations are an important but often neglected aspect of data reuse \citep{Bornatici2023, Vannan2020}. Guidelines for data sharing have emphasized the need to document the specific data versions, access times, and sources \citep{Bornatici2023, DCSG11} with some additional requirements for querying and filtering dynamically evolving datasets in a reproducible manner \citep{Rauber2015, Rauber2021}. Ideally, datasets should be cited in a standardized format, similarly to other sources \citep{Bornatici2023}. This process should be simplified as much as possible.
...
% The \CRANpkg{pxweb} package is now filling this gap [CLOSELY RELATED PKGS SHOULD BE CITED HERE?]. Following its first CRAN release in 2014, the \CRANpkg{pxweb}, several contributors and feedback from the user community have supported the package development. \citep{Schlegel2023,Hirvonen2021,Solt2020}
...
listed in
https://www.scb.se/en/services/statistical-programs-for-px-files/px-web/pxweb-examples}. We have systematically gone through all statistical databases utilizing Px-Web technology and added all of those that had API access enabled, which was the majority of them. Additionally, we have added also APIs not listed in SCB website if we have come across them from other sources.
...
% \include{api}
\toprule
ISO-2 & API - Service Provider \\
\hline
SE & SCB - Statistics Sweden \\
SE & SJV - The Swedish Agricultural Agency \\
SE & Fohm - The Public Health Agency of Sweden \\
SE & KONJ - The Swedish National Institute of Economic Research \\
SE & The Swedish national institute of economic research, forecast database \\
SE & Linköping municipality \\
SE & VG-region - Västra Götaland Region \\
FI & StatFin - Statistics Finland \\
FI & LUKE - Natural Resources Institute Finland \\
FI & Vero - Finnish Tax Administration \\
FI & Trafi - Finnish Transport Safety Agency \\
FI & Rudolf - Visit Finland \\
FI & Helsinki Regional Statistics \\
AX / FI & ASUB - Statistics Åland \\
IS & Statice - Statistics Iceland \\
% IS & RSV - Icelandic Centre for Retail Studies \\ Does not exist anymore
NO & SSB - Statistics Norway \\
GL / DK & Statbank Greenland \\
FO / DK & Hagstovan - Statistics Faroe Islands \\
MK & Makstat - State Statistical Office of the Republic of Macedonia \\
LV & CSB - Latvia - Official Statistics \\
MD & Statistica - Statistics Moldova \\
CH & BFS - Statistics Switzerland \\
KV & RKS - Statistics Kosovo \\
LU & Grande-Region - Portail statistique de la Grande Region \\
EE & Estonia - official statistics \\
SI & SiStat - Statistical Office of the Republic of Slovenia \\
\hline
ORG & Nordic Statistics Database \\
ORG & UNECE - United Nations Economic Commission for Europe \\
\bottomrule
\end{tabular}
...
\end{table}
% WIP: Add recently added missing APIs here
...
In recent years, more attention has been given to the importance of attributing software similarly to attributing other research output, such as books or journals \citep{Smith2016software}. However, challenges remain in motivating authors to actually cite the software they used \citep{Katz2018}. The resulting lack of attribution can result differing social norms and practices in different fields and scholars being more accustomed to citing papers has potentially led to writing software papers \citep{Smith2016software}. Another reason lies in journals, conferences and publishers not requiring authors to cite software used in research \citep{Katz2018}. More easy-to-use technologies can facilitate more widespread adoption of technologies \citep{Davis1989} and R's built in citation() function, additional R packages and software created to serve such need (see e.g. \cite{Hernangomez2021, Druskat2021}) and R package templates that include CITATION file with the package by default facilitate easier attribution of R software package developers. This combined infrastructure of good, enforced practices and easy-to-use software functionalities contribute in general to achieving the virtues of reproducibility, findability, scientific transparency, and reusability \citep{Hausman2019}.

Previous work on data citation practices have emphasized the need for 

Citing data is at least equally as important citing software used. 
...
Whereas \CRANpkg{pxweb} has been designed to access the PX-Web API, this should not be confused with the related PC-Axis file format (typically abbreviated as '.px'). We anticipate that the more flexible PC Axis API is gradually taking over the PC-Axis file format as the means of data sharing for official statistics. In addition to downloading data in .px format, websites built with PxWeb technology support also downloading data as XML, Excel-files, various delimiter separated formats, and JSON/JSON-stat/JSON-stat2 files. Those who need to access and parse legacy .px files can have a look at the independently developed pxR package, which is currently maintained in Github \url{https://github.com/cjgb/pxR}.

%Examples of known case studies etc. that the package has enabled

Whereas the methods can be used with any PX-WEB API that is locally accessible, an increasing number of the official statistical resources are open access. The statistical authorities in many Nordic countries have invested in open data sharing, which supports use cases by governmental authorities, companies, and citizen scientists. The package has been used, for instance, as the data retrieval method for population statistics from different postal code areas when studying the connection between the prevalence of electronic gambling machines and population level socio-economic indicators \citep{Raisamo2019}, municipality-level employment statistics when studying displacement effects of active labour market policies \citep{Hirvonen2021}, and as one of the data retrieval methods for building the cross-national Standardized World Income Inequality Database \citep{Solt2020}. % Other citations: Schlegel2023 is preprint that probably didn't even use pxweb
