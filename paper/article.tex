\title{Opening official statistics with the pxweb package}

\author{Måns Magnusson, Uppsala University\\Leo Lahti, University of Turku}
%\And Markus Kainu\\Plus Affiliation 

\maketitle

% an abstract and keywords
\abstract{An increasing number of governmental and other organizations are sharing statistical information as open data through machine-readable interfaces online. Common data representation standards can greatly facilitate the use of such information, and the development of downstream methods for statistical data analysis. Moreover, efficient and transparent use of official statistics relies on our ability to standardize the full data science workflows spanning, data access, analysis, and reproducible reporting. We introduce the pxweb R package, which provides comprehensively tested programmatic tools to access structured statistical information shared in the PX-Web format. This standardized format is used by dozens of statistical authorities across Europe, and the pxweb R package allows seamless integration of these information sources with the latest statistical and machine learning techniques that are available via the broader R ecosystem. We provide an overview of the implemented methods, provide examples on possible use cases, and conclude by discussing the limitations and further development opportunities of the pxweb R package.}


\section{Introduction}

Mention Finland API policies

Motivation here. Why do we need this package? The open data pipeline, access, use and cite.

How old and mature package
...
In summary, the \CRANpkg{pxweb} package provides custom tools for open statistical data resources provided through the PX-WEB API. Currently, the pxweb package provides seamless algorithmic access from the R environment to dozens of data collections from national authorities in countries such as Estonia, Iceland, Finland, Norway, Sweden, The Netherlands, and elsewhere. Seamless integration with other data analysis tools is facilitated by support for features such as cache, date formatting, tidy data principles \citep{wickham2014}, and the \pkg{tibble} \citep{tibble} data format. In this article, we provide an overview of the functionality and use cases based on the current CRAN release version (0.8). The comprehensive on-line documentation, which is available via the package homepage\footnote{\url{http://ropengov.github.io/pxweb}}, includes simple examples for individual functions, generic tutorials, and links to more advanced case studies. Moreover, the package is following best practices in open source software development such as version control, automated unit tests, continuous integration, and collaborative development \citep{PerezRiverol2016}.
