...
PX-Web query handling. The table ``Population by region, marital
status, age and sex. Year 1968--2024'' is available through the PX-Web
API version 2 as table \code{TAB638}. It contains annual population
counts by region, marital status, single-year age, sex, content
variable, and year.
...
cells. This makes the table a useful example of why metadata-aware
query validation and request splitting are needed for reproducible
official-statistics workflows.

The replication script \code{case-study.R} uses a large but manageable
query. It requests the county-level population counts for all 21
Swedish counties, all marital-status categories, single-year ages from
0 to 100+, both sexes, the population count content variable, and all
years from 1968 to 2024. This query contains 967,176 cells, which is
well above the provider-side limit. The \pkg{pxweb} package validates
the query, splits it into seven batches, downloads the pieces, and
combines them into one \code{data.frame}. The script caches the
downloaded data locally so that the figure can be regenerated without
repeated API calls.
...
The first two panels aggregate counties, marital statuses, ages, and
sexes to show the growth of the population and the increasing share of
the population aged 65 years or older. The lower panels compare the age
and sex structure in 1968 and 2024. The example therefore demonstrates
the main package workflow: a PX-Web API version 2 table is queried
programmatically, a request larger than the API cell limit is split and
combined by \pkg{pxweb}, and the resulting data are transformed into
standard \proglang{R} objects for analysis and visualization.
