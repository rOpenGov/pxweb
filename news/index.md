...
- Return better error messages when using as.data.frame() for pxweb
  metadata objects.
...
- exposed pxweb_parse_response() and is_pxweb_response() to the package
  API for more advanced users that want to make their own `httr` calls
  to a PXWEB API.
