...
test_that(desc = "pxweb_query object", {
  ...
  json_query <- file.path(system.file(package = "pxweb"), "extdata", "examples", "statfin_vaenn_139e_query.json")
...
test_that(desc = "split pxweb_query object", {
  ...
  load(test_path(file.path("test_data", "pxm1_test.rda")))
  pxmd <- px_meta
  px <- list(config = list(max_values_to_download = 110000))
  class(px) <- c("pxweb", "list")
